...
namespace PerkinElmer.Simplicity.Data.Version16.DataAccess.Postgresql.AuditTrail
{
	...
	internal static class SqlAuditTrailLogs
	{
		internal static string SqlSelect =
            ...
            $"{AuditTrailLogsTableDefinition.UniqueIdColumn}," +
        ...
        internal static string SqlSelectByUniqueId =
            SqlSelectFromTable +
            $"where {AuditTrailLogsTableDefinition.UniqueIdColumn} =  '{{0}}' ";
        ...
        internal static string UpdateVersionInfoByUniqueId =
            $"update {AuditTrailLogsTableDefinition.TableName} " +
            "set " +
            $"{AuditTrailLogsTableDefinition.EntityIdColumn} = @{AuditTrailLogsTableDefinition.EntityIdColumn}, " +
            $"{AuditTrailLogsTableDefinition.EntityTypeColumn} = @{AuditTrailLogsTableDefinition.EntityTypeColumn}, " +
	...
	internal class AuditTrailDao
	{
			...
			try
			{
                ...
                if (!string.IsNullOrEmpty(searchText))
				{
					...
					whereClause += " AND( " +
								...
								$" OR ''||{AuditTrailLogsTableDefinition.IdColumn}" + " LIKE " + searchText +
								")";
				}
        ...
        internal AuditTrailLogEntry GetAuditTrailByUniqueId(IDbConnection connection, Guid uniqueLogEntryId)
        {
            if (uniqueLogEntryId == null)
                throw new ArgumentException(nameof(uniqueLogEntryId));

            try
            {
                return connection.QueryFirstOrDefault<AuditTrailLogEntry>(string.Format(SqlAuditTrailLogs.SqlSelectByUniqueId, uniqueLogEntryId));
            }
            catch (Exception ex)
            {
                Log.Error("Error in GetAuditTrailByUniqueId method", ex);
                throw;
            }
        }
		...
		internal AuditTrailLogEntry CreateLog(IDbConnection connection, AuditTrailLogEntry logEntry)
		{
			...
			try
			{
				logEntry.Id = connection.ExecuteScalar<long>(SqlAuditTrailLogs.SqlInsertAndReturnId, logEntry);
				return logEntry;
			}
			catch (Exception ex)
			{
        ...
        internal bool UpdateLogEntryVersionInfo(IDbConnection connection, Guid logEntryUniqueId, AuditTrailLogEntry logEntry)
        {
            if (logEntryUniqueId == null||
                logEntry == null)
                throw new ArgumentException("Parameter Incorrect!");

            try
            {
                var result = connection.Execute(string.Format(SqlAuditTrailLogs.UpdateVersionInfoByUniqueId, logEntryUniqueId), logEntry);
                return result == 1;
            }
            catch (Exception ex)
            {
