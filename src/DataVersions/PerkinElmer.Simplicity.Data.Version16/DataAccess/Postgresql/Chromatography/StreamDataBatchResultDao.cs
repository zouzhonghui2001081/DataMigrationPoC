...
namespace PerkinElmer.Simplicity.Data.Version16.DataAccess.Postgresql.Chromatography
{
	internal class StreamDataBatchResultDao
	{
		...
		public IList<StreamDataBatchResult> GetStreamInfo(IDbConnection connection, Guid batchRunGuid)
		{
			...
			var batchRun = batchRunDao.GetBatchRunByGuid(connection, batchRunGuid);
			if (batchRun == null)
				return new List<StreamDataBatchResult>();
            ...
            return connection.Query<StreamDataBatchResult>(
                ...
                $"FROM {TableName} " +
                $"WHERE {TableName}.{BatchRunIdColumn} = @BatchRunId",
				new { BatchRunId = batchRun.Id}).ToList();
        }
